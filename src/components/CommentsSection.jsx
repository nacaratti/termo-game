...
import { LogIn, LogOut, Send, Trash2, Loader2, MessageSquare, ChevronRight } from 'lucide-react';
...
const AnonCommentForm = ({ onPosted, onSignIn, error: parentError }) => {
  ...
  if (status === 'pending') {
    return (
      <div className="mb-6 rounded-lg p-4 text-center" style={{ backgroundColor: SURF }}>
        <p className="text-sm text-zinc-400">Obrigado pelo comentário!</p>
        <p className="text-xs text-zinc-600 mt-1">Seu comentário será exibido após aprovação.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mb-6 space-y-3">
      {/* Botão Google */}
      <div className="flex items-center justify-between gap-3 p-3 rounded-lg" style={{ backgroundColor: SURF }}>
      ...
      {parentError && <p className="text-red-400 text-xs">{parentError}</p>}

      {/* Form anônimo */}
      <div className="rounded-lg overflow-hidden border" style={{ borderColor: BDR2 }}>
        <button
          onClick={() => setOpen(o => !o)}
          className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-zinc-500 hover:text-white transition-colors"
        >
          <MessageSquare className="h-4 w-4 shrink-0" />
          <span className="flex-1 text-left">Ou comente sem login</span>
          <ChevronRight className={`h-4 w-4 shrink-0 transition-transform ${open ? 'rotate-90' : ''}`} />
        </button>
        {open && (
          <form onSubmit={handleSubmit} className="border-t px-3 pb-3 pt-2.5 flex flex-col gap-2" style={{ borderColor: BDR2 }}>
            <input
              value={authorName}
            ...
            <textarea
              ...
              rows={3}
              className="w-full rounded-lg px-3 py-2 text-sm text-white placeholder:text-zinc-600 border outline-none resize-none focus:border-zinc-500 transition-colors"
              style={{ backgroundColor: SURF, borderColor: BDR2 }}
            />
    ...
    <div>
      ...
      >
        ...
        {user ? (
          ...
          <AnonCommentForm onPosted={fetchComments} onSignIn={handleSignIn} error={error} />
        )}
